...
class TestCommandSummaryPersistence:
    def test_command_created_without_summary(self, db_session):
        ...
        project = Project(
            name="cmd-test-project", slug="cmd-test-project", path="/test/cmd-path"
        )
        db_session.add(project)
        db_session.flush()
    ...
    def test_command_summary_persisted(self, db_session):
        ...
        project = Project(
            name="cmd-test-project-2",
            slug="cmd-test-project-2",
            path="/test/cmd-path-2",
        )
        db_session.add(project)
        db_session.flush()
        ...
        command.completion_summary = (
            "Implemented JWT auth with refresh tokens. All tests passing."
        )
    ...
    def test_command_summary_with_turns(self, db_session):
        ...
        project = Project(
            name="cmd-test-project-3",
            slug="cmd-test-project-3",
            path="/test/cmd-path-3",
        )
        db_session.add(project)
        db_session.flush()
        ...
        assert (
            fetched_command.turns[1].summary == "Agent fixed and tested the login bug."
        )

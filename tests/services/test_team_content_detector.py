...
class TestXMLDetection:
    ...
    def test_task_notification(self):
        assert is_team_internal_content(
            "<task-notification>Agent completed work</task-notification>"
        )

    def test_system_reminder(self):
        assert is_team_internal_content(
            "<system-reminder>Context info here</system-reminder>"
        )
    ...
    def test_task_notification_mid_text_is_not_internal(self):
        ...
        assert not is_team_internal_content(
            "Some prefix <task-notification>payload</task-notification> suffix"
        )

    def test_tag_mentioned_in_backticks_is_not_internal(self):
        """Agent discussing tags in prose should not be flagged."""
        assert not is_team_internal_content(
            "Detects `<task-notification>` and `<system-reminder>` XML tags"
        )
    ...
    def test_system_reminder_with_attributes(self):
        assert is_team_internal_content(
            '<system-reminder foo="bar">content</system-reminder>'
        )
...
class TestIdleJSON:
    ...
    def test_idle_with_extra_fields(self):
        msg = json.dumps(
            {"type": "idle", "agent": "researcher", "timestamp": "2026-02-16T12:00:00Z"}
        )
        assert is_team_internal_content(msg)
...
class TestPersonaInjectionDetection:
    ...
    def test_normal_user_text_returns_false(self):
        assert (
            is_persona_injection("Fix the login bug in the authentication module")
            is False
        )
    ...
    def test_text_mentioning_persona_but_not_matching_pattern(self):
        text = (
            "The user said 'You are Con. Read the following skill and experience' but it was "
            + "x" * 200
        )
        assert is_persona_injection(text) is False

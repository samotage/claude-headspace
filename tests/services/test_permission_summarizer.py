...
class TestSummarizePermissionCommand:
    ...
    def test_bash_curl_with_url(self):
        result = summarize_permission_command(
            "Bash", {"command": "curl -s http://localhost:5055/dashboard"}
        )
        assert result == "Bash: curl from localhost:5055"

    def test_bash_curl_https(self):
        result = summarize_permission_command(
            "Bash", {"command": "curl https://api.example.com/v1/data"}
        )
    ...
    def test_bash_tail(self):
        result = summarize_permission_command(
            "Bash", {"command": "tail -f /var/log/syslog"}
        )
        assert result == "Bash: read syslog"

    def test_bash_head(self):
        result = summarize_permission_command(
            "Bash", {"command": "head -n 20 src/app.py"}
        )
    ...
    def test_bash_pip_install(self):
        result = summarize_permission_command(
            "Bash", {"command": "pip install -e '.[dev]'"}
        )
    ...
    def test_bash_cp(self):
        result = summarize_permission_command(
            "Bash", {"command": "cp file.txt backup.txt"}
        )
    ...
    def test_bash_sed(self):
        result = summarize_permission_command(
            "Bash", {"command": "sed -i 's/old/new/g' config.yaml"}
        )
    ...
    def test_read_tool(self):
        result = summarize_permission_command(
            "Read", {"file_path": "/Users/sam/project/src/app.py"}
        )
        assert result == "Read: src/app.py"

    def test_write_tool(self):
        result = summarize_permission_command(
            "Write", {"file_path": "/tmp/claude-501/scratchpad/output.txt"}
        )
        assert result == "Write: (temp) output.txt"

    def test_edit_tool(self):
        result = summarize_permission_command(
            "Edit", {"file_path": "/Users/sam/project/config.yaml"}
        )
    ...
    def test_websearch(self):
        result = summarize_permission_command(
            "WebSearch", {"query": "flask sse tutorial"}
        )
...
class TestClassifySafety:
    ...
    def test_webfetch_safe_read(self):
        assert (
            classify_safety("WebFetch", {"url": "https://example.com"}) == "safe_read"
        )
    ...
    def test_bash_grep_safe_read(self):
        assert (
            classify_safety("Bash", {"command": "grep -r 'pattern' ."}) == "safe_read"
        )

    def test_bash_curl_get_safe_read(self):
        assert (
            classify_safety("Bash", {"command": "curl https://api.example.com"})
            == "safe_read"
        )
    ...
    def test_bash_curl_post_safe_write(self):
        assert (
            classify_safety(
                "Bash", {"command": "curl -X POST https://api.example.com -d 'data'"}
            )
            == "safe_write"
        )
    ...
    def test_bash_git_commit_safe_write(self):
        assert (
            classify_safety("Bash", {"command": "git commit -m 'msg'"}) == "safe_write"
        )

    def test_bash_git_push_safe_write(self):
        assert (
            classify_safety("Bash", {"command": "git push origin main"}) == "safe_write"
        )

    def test_bash_git_push_force_destructive(self):
        assert (
            classify_safety("Bash", {"command": "git push --force origin main"})
            == "destructive"
        )
    ...
    def test_bash_unknown_command(self):
        assert (
            classify_safety("Bash", {"command": "some-custom-tool arg1"}) == "unknown"
        )
    ...
    def test_bash_npm_install_safe_write(self):
        assert (
            classify_safety("Bash", {"command": "npm install express"}) == "safe_write"
        )
    ...
    def test_compound_mixed_read_write(self):
        ...
        assert (
            classify_safety("Bash", {"command": "ls -la; python run.py"})
            == "safe_write"
        )
    ...
    def test_compound_with_destructive(self):
        ...
        assert (
            classify_safety("Bash", {"command": "ls -la; rm -rf /tmp/foo"})
            == "destructive"
        )
    ...
    def test_compound_and_operator(self):
        ...
        assert (
            classify_safety("Bash", {"command": "git status && git log --oneline"})
            == "safe_read"
        )
    ...
    def test_compound_and_with_write(self):
        ...
        assert (
            classify_safety("Bash", {"command": "git add . && git commit -m 'msg'"})
            == "safe_write"
        )
    ...
    def test_compound_unknown_elevates_to_safe_write(self):
        ...
        assert (
            classify_safety("Bash", {"command": "ls -la; some-unknown-tool arg1"})
            == "safe_write"
        )
    ...
    def test_option_label_allow_reading(self):
        ...
        assert (
            classify_safety(
                "Bash",
                {"command": "find /Users/samotage -name '*.py'"},
                pane_context={
                    "options": [
                        {"label": "Yes, allow reading from samotage/ from this project"}
                    ]
                },
            )
            == "safe_read"
    ...
    def test_option_label_allow_writing(self):
        ...
        assert (
            classify_safety(
                ...
                pane_context={
                    "options": [
                        {"label": "Yes, allow writing to samotage/ from this project"}
                    ]
                },
            )
            == "safe_write"
    ...
    def test_option_label_no_match_falls_through(self):
        ...
        assert (
            classify_safety(
                "Bash",
                {"command": "ls -la"},
                pane_context={"options": [{"label": "Yes, allow this action"}]},
            )
            == "safe_read"
        )
    ...
    def test_option_label_empty_options_falls_through(self):
        ...
        assert (
            classify_safety("Bash", {"command": "ls -la"}, pane_context={"options": []})
            == "safe_read"
        )
    ...
    def test_option_label_none_options_falls_through(self):
        ...
        assert (
            classify_safety(
                "Bash", {"command": "cat /etc/hosts"}, pane_context={"options": None}
            )
            == "safe_read"
        )
    ...
    def test_pane_context_without_options_falls_through(self):
        ...
        assert (
            classify_safety(
                ...
                pane_context={"description": "some description"},
            )
            == "safe_read"
        )

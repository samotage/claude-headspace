...
def setup_data(app, db_session):
    ...
    persona_a = Persona(name="Alice", role_id=role.id, role=role, persona_type_id=pt_internal.id, status="active")
    persona_b = Persona(name="Bob", role_id=role.id, role=role, persona_type_id=pt_internal.id, status="active")
    persona_ext = Persona(name="ExtUser", role_id=role.id, role=role, persona_type_id=pt_external.id, status="active")
    ...
    agent_a = Agent(session_uuid=uuid4(), project_id=project.id, persona_id=persona_a.id)
    agent_b = Agent(session_uuid=uuid4(), project_id=project.id, persona_id=persona_b.id)
    ...
    return {
        ...
        "persona_ext": persona_ext, "agent_a": agent_a,
        "agent_b": agent_b, "project": project, "role": role,
    }
...
class TestCreateChannel:
    def test_create_channel_success(self, channel_service, setup_data):
        channel = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="test-channel",
            channel_type="workshop", description="Test description",
        )
    ...
    def test_create_channel_with_members(self, channel_service, setup_data):
        channel = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="team-channel",
            channel_type="workshop", member_slugs=[setup_data["persona_b"].slug],
        )
        memberships = ChannelMembership.query.filter_by(channel_id=channel.id).all()
    ...
    def test_create_channel_no_capability(self, channel_service, setup_data):
        with pytest.raises(NoCreationCapabilityError) as exc_info:
            channel_service.create_channel(
                creator_persona=setup_data["persona_ext"], name="fail-channel",
...
class TestListChannels:
    def test_list_member_scoped(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="my-channel",
            channel_type="workshop",
        )
    ...
    def test_list_all_visible(self, channel_service, setup_data):
        channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="visible",
            channel_type="workshop",
        )
        result = channel_service.list_channels(setup_data["persona_b"], all_visible=True)
        assert len(result) >= 1
...
class TestGetChannel:
    def test_get_channel_success(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="get-test",
            channel_type="review",
        )
...
class TestUpdateChannel:
    def test_update_by_chair(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="update-test",
            channel_type="workshop",
        )
        updated = channel_service.update_channel(
            ch.slug, setup_data["persona_a"],
            description="Updated desc", intent_override="New intent",
        )
    ...
    def test_update_by_non_chair_fails(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="update-fail",
            channel_type="workshop",
        )
        with pytest.raises(NotChairError):
            channel_service.update_channel(ch.slug, setup_data["persona_b"], description="Nope")


class TestCompleteChannel:
    def test_complete_by_chair(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="complete-test",
            channel_type="workshop",
        )
    ...
    def test_complete_by_non_chair_fails(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="complete-fail",
            channel_type="workshop",
        )
        with pytest.raises(NotChairError):
...
class TestArchiveChannel:
    def test_archive_complete_channel(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="archive-test",
            channel_type="workshop",
        )
    ...
    def test_archive_active_channel_fails(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="archive-fail",
            channel_type="workshop",
        )
...
class TestAddMember:
    def test_add_member_success(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="add-test",
            channel_type="workshop",
        )
        ...
        msgs = Message.query.filter_by(channel_id=ch.id, message_type=MessageType.SYSTEM).all()
        assert any("Bob joined" in m.content for m in msgs)

    def test_add_member_already_member(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="dup-test",
            channel_type="workshop",
        )
        channel_service.add_member(ch.slug, setup_data["persona_b"].slug, setup_data["persona_a"])
        with pytest.raises(AlreadyMemberError):
            channel_service.add_member(ch.slug, setup_data["persona_b"].slug, setup_data["persona_a"])

    def test_add_member_closed_channel(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="closed-test",
            channel_type="workshop",
        )
        channel_service.complete_channel(ch.slug, setup_data["persona_a"])
        with pytest.raises(ChannelClosedError):
    ...
    def test_add_member_spin_up(self, channel_service, setup_data):
        ...
        db.session.commit()
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="spinup-test",
            channel_type="workshop",
        )
        with patch("claude_headspace.services.agent_lifecycle.create_agent") as mock_create:
...
class TestAgentChannelConflict:
    def test_conflict_detected(self, channel_service, setup_data):
        ch1 = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="channel-1",
            channel_type="workshop",
        )
        channel_service.add_member(ch1.slug, setup_data["persona_b"].slug, setup_data["persona_a"])
...
class TestLeaveChannel:
    def test_leave_success(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="leave-test",
            channel_type="workshop", member_slugs=[setup_data["persona_b"].slug],
        )
    ...
    def test_last_member_leave_auto_completes(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="autocomp-test",
            channel_type="workshop",
        )
...
class TestTransferChair:
    def test_transfer_success(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="transfer-test",
            channel_type="workshop", member_slugs=[setup_data["persona_b"].slug],
        )
        channel_service.transfer_chair(ch.slug, setup_data["persona_b"].slug, setup_data["persona_a"])
        a_m = ChannelMembership.query.filter_by(channel_id=ch.id, persona_id=setup_data["persona_a"].id).first()
        b_m = ChannelMembership.query.filter_by(channel_id=ch.id, persona_id=setup_data["persona_b"].id).first()
        assert a_m.is_chair is False
        assert b_m.is_chair is True

    def test_transfer_by_non_chair(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="transfer-fail",
            channel_type="workshop", member_slugs=[setup_data["persona_b"].slug],
        )
        with pytest.raises(NotChairError):
            channel_service.transfer_chair(ch.slug, setup_data["persona_a"].slug, setup_data["persona_b"])


class TestMuteUnmute:
    def test_mute_and_unmute(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="mute-test",
            channel_type="workshop",
        )
...
class TestSendMessage:
    def test_send_message_success(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="msg-test",
            channel_type="workshop",
        )
    ...
    def test_first_message_activates_channel(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="activate-test",
            channel_type="workshop",
        )
    ...
    def test_send_to_closed_channel(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="closed-msg-test",
            channel_type="workshop",
        )
        channel_service.complete_channel(ch.slug, setup_data["persona_a"])
        with pytest.raises(ChannelClosedError):
    ...
    def test_send_by_non_member(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="non-member-msg",
            channel_type="workshop",
        )
    ...
    def test_system_message_type_rejected(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="system-reject",
            channel_type="workshop",
        )
        with pytest.raises(ValueError):
            channel_service.send_message(ch.slug, "hack", setup_data["persona_a"], message_type="system")


class TestGetHistory:
    def test_history_success(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="history-test",
            channel_type="workshop",
        )
    ...
    def test_history_left_member_can_read(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="left-history",
            channel_type="workshop", member_slugs=[setup_data["persona_b"].slug],
        )
    ...
    def test_history_non_member_rejected(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="no-access",
            channel_type="workshop",
        )
        with pytest.raises(NotAMemberError):
    ...
    def test_history_pagination(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="paginate-test",
            channel_type="workshop",
        )
        for i in range(5):
...
class TestContextBriefing:
    def test_briefing_format(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="briefing-test",
            channel_type="workshop",
        )
    ...
    def test_briefing_empty_channel(self, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="empty-briefing",
            channel_type="workshop",
        )
        briefing = channel_service._generate_context_briefing(ch)
        assert briefing == ""
...
class TestSSEBroadcasting:
    def test_broadcast_message(self, app, channel_service, setup_data):
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="sse-msg-test",
            channel_type="workshop",
        )
        mock_broadcaster = MagicMock()
        app.extensions["broadcaster"] = mock_broadcaster
        channel_service.send_message(ch.slug, "SSE test", setup_data["persona_a"])
        calls = [c for c in mock_broadcaster.broadcast.call_args_list if c[0][0] == "channel_message"]
        assert len(calls) >= 1
    ...
    def test_broadcast_update(self, app, channel_service, setup_data):
        ...
        app.extensions["broadcaster"] = mock_broadcaster
        ch = channel_service.create_channel(
            creator_persona=setup_data["persona_a"], name="sse-update-test",
            channel_type="workshop",
        )
        calls = [c for c in mock_broadcaster.broadcast.call_args_list if c[0][0] == "channel_update"]
        assert len(calls) >= 1

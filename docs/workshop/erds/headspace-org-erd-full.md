...
### Epic 9 Workshop Updates — Decisions 0A.1–0A.7 (3 March 2026)

- **Handoff filename format resolved (Decision 0A.1).** New format: `{timestamp}_{summary-slug}_{agent-id:NNN}.md`. Underscore separators between three sections. Summary slug is kebab-case, max 60 chars, generated by departing agent. Legacy format accepted (mixed formats in directory). No DB summary column — filename is the summary.
- **Startup detection resolved (Decision 0A.2).** On agent creation after persona assignment, scan `data/personas/{slug}/handoffs/` for `.md` files. Sort by filename, take most recent 3. Present via synthetic SSE turns. No staleness threshold. Fires even for HandoffExecutor-created agents.
- **Synthetic injection resolved (Decision 0A.3).** Dashboard-only SSE event (`synthetic_turn` type). Agent doesn't see it. Rendered as visually distinct bubbles with copyable file paths. General-purpose mechanism (reusable for other system context).
- **Operator rehydration resolved (Decision 0A.4).** Manual: operator selects handoff from listing, copies path, pastes into message to agent with natural language instruction. No consumption tracking, no card status change.
- **Handoff instruction changes resolved (Decision 0A.6).** `generate_handoff_file_path()` produces path with `<insert-summary>` placeholder. Polling thread glob fallback: `{timestamp}_*_{agent_tag}.md`. 6-section document structure unchanged.
- **CLI integration resolved (Decision 0A.7).** `flask org persona handoffs <slug>` under existing persona subgroup. Filesystem-only query. Plain text output: timestamp, summary, agent ID per line.

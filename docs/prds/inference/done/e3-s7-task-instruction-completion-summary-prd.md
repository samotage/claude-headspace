...
## Product Requirements Document (PRD) — Command Instruction & Completion Summary

**Project:** Claude Headspace
**Scope:** Command model enrichment, summarisation prompt rebuild, agent card display
...
Commands in Claude Headspace currently produce meaningless AI-generated summaries because the summarisation prompts lack context. The command summary prompt receives only timestamps, turn count, and the final turn's text — with no knowledge of what was originally commanded. Turn summaries suffer similarly: when turn text is empty or the prompt lacks command context, the LLM produces filler like "The agent indicated the intention to complete the given task."

This PRD introduces two new command-level concepts: an **instruction** (LLM-derived summary of the initiating command) and a **completion summary** (LLM-derived summary of task outcome with full context). It also rebuilds the turn and command summarisation prompts to produce useful, context-aware summaries, and updates the agent card UI to display both the command instruction and the latest contextual turn summary.
...
The E3-S2 sprint added turn and command summarisation via the inference layer. The E3-S6 sprint added content pipeline support to extract real transcript text into turns. However, the summarisation prompts were written before real content was flowing, and they produce low-quality outputs:

- **Command summary prompt** (`_build_task_prompt`): only passes timestamps, turn count, and `turns[-1].text`. No original command context. When the final turn text is empty (timing issue), the LLM parrots timestamps back.
- **Turn summary prompt** (`_build_turn_prompt`): passes `turn.text`, actor, and intent. When text is None/empty, the LLM generates filler from metadata alone. No command-level context is provided regardless.
...
- **Line 1:** "Refactor the authentication middleware to use JWT tokens" (command instruction)
...
- New `instruction` field on Command model — LLM-derived summary of the initiating USER COMMAND turn
- Rename existing `summary` field to `completion_summary` on Command model
- Rebuild command completion summary prompt: primary input is the agent's final message that triggered COMPLETE, with the command instruction as context
- Rebuild turn summarisation prompts with intent-aware templates and command instruction context
...
- Agent card UI: two-line display showing command instruction and latest contextual turn summary
- SSE events updated to push instruction field
- Alembic migration for field rename and new field
- Update all service, route, template, and test references from `command.summary` to `command.completion_summary`
...
- Command state machine changes
- Historical backfill of existing commands with new instruction/completion_summary
...
3. Command completion summaries describe what was accomplished relative to what was asked — not timestamps or turn counts
4. Turn summaries for empty-text turns are not generated (no filler summaries from metadata alone)
5. Turn summaries include command instruction context, producing summaries that relate to the task being worked on
6. Agent cards display the command instruction as the primary line when a command is active
...
**FR1:** The Command model has a new `instruction` field (Text, nullable) that stores the LLM-derived summary of what the user commanded.

**FR2:** The Command model has a new `instruction_generated_at` field (DateTime, nullable) that records when the instruction summary was generated.

**FR3:** The existing Command `summary` field is renamed to `completion_summary`. The existing `summary_generated_at` field is renamed to `completion_summary_generated_at`.

**FR4:** An Alembic migration handles the field rename and new field addition.

### Instruction Generation

**FR5:** When a command is created from a USER COMMAND turn, instruction summarisation is triggered asynchronously. The prompt receives the full text of the user's command and produces a 1-2 sentence summary of what was instructed.

**FR6:** The instruction summary is persisted to `command.instruction` and broadcast via SSE as an `instruction_summary` event (or similar).

### Completion Summary

**FR7:** The command completion summary prompt is rebuilt. Its primary inputs are:
- The command instruction (what was commanded)
...
- **COMPLETION:** Summarise what the agent accomplished (with command instruction context)
- **PROGRESS:** Summarise what progress the agent has made
- **ANSWER:** Summarise what information the user provided
- **END_OF_COMMAND:** Summarise the final outcome of the task

**FR11:** Turn summarisation includes the command instruction as context in the prompt (when available), so summaries are grounded in what the command is about.
...
**FR13:** The agent card displays the command instruction as the primary information line when a command is active.
...
**FR15:** When no command is active (IDLE state), the agent card displays an appropriate idle message (existing behaviour preserved).
...
**FR17:** All references to `command.summary` across services, routes, templates, SSE handlers, and tests are updated to `command.completion_summary`. All references to `command.summary_generated_at` are updated to `command.completion_summary_generated_at`.
...
### Agent Card — Active Command State
...
│─────────────────────────────────────────────│
│ Refactor auth middleware to use JWT tokens  │  ← Command instruction (NEW)
...
│─────────────────────────────────────────────│
│ Refactor auth middleware to use JWT tokens  │  ← Command instruction
...
│─────────────────────────────────────────────│
│ No active command                              │  ← Existing idle message
│─────────────────────────────────────────────│

...
- [ ] 2.1 Create `ApiCallLog` model in `src/claude_headspace/models/api_call_log.py`
...
- [ ] 2.2 Create Alembic migration for `api_call_logs` table
- [ ] 2.3 Register model import in app factory

### Capture Middleware/Service

- [ ] 2.4 Create `ApiCallLogger` service in `src/claude_headspace/services/api_call_logger.py`
...
- [ ] 2.5 Register `ApiCallLogger` in `app.py` app factory (attach before/after request hooks)

### API Endpoints

- [ ] 2.6 Add `api_log_page` route (`/logging/api`) to `logging.py` blueprint
  - Render `logging_api.html` template
- [ ] 2.7 Add `GET /api/logging/api-calls` endpoint to `logging.py`
...
- [ ] 2.8 Add `GET /api/logging/api-calls/filters` endpoint to `logging.py`
  - Return distinct endpoint paths, HTTP methods, status categories, auth statuses from existing records
- [ ] 2.9 Add `DELETE /api/logging/api-calls` endpoint to `logging.py`
  - Require `X-Confirm-Destructive: true` header
  - Delete all ApiCallLog records, return count

### UI Template & JavaScript

- [ ] 2.10 Create `templates/logging_api.html` following `logging_inference.html` pattern
...
- [ ] 2.11 Create `static/js/logging-api.js` following `logging-inference.js` pattern
...
- [ ] 2.12 Update `templates/partials/_logging_tabs.html` to add API tab
...
- [ ] 3.1 Create `tests/services/test_api_call_logger.py`
...
- [ ] 3.2 Create `tests/routes/test_logging_api.py`

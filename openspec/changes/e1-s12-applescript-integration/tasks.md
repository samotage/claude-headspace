...
- [ ] Review existing Agent model and iterm_pane_id field
- [ ] Review existing Flask app structure for blueprint registration
- [ ] Research iTerm2 AppleScript API documentation
- [ ] Plan service architecture

## Phase 2: Implementation

### AppleScript Focus Service (FR6-FR11)
- [ ] Create src/claude_headspace/services/iterm_focus.py
- [ ] Implement focus_iterm_pane(pane_id) function
...
- [ ] Implement subprocess timeout (2 seconds) for NFR2
- [ ] Handle AppleScript execution errors
- [ ] Detect permission denied errors (FR15)
- [ ] Detect iTerm2 not running (FR14)
- [ ] Detect pane not found (FR13)

### API Endpoint (FR1-FR5)
- [ ] Create src/claude_headspace/routes/focus.py blueprint
- [ ] Implement POST /api/focus/<agent_id> endpoint
...
- [ ] Include fallback_path in all error responses for existing agents (FR17)
- [ ] Format actionable error messages (FR15, FR18)

### Event Logging (FR19-FR20)
- [ ] Log focus_attempted event on each focus attempt
- [ ] Include payload: agent_id, pane_id, outcome, error_type, latency_ms

## Phase 3: Testing

- [ ] Test POST /api/focus returns 200 on success
- [ ] Test POST /api/focus returns 404 for unknown agent
- [ ] Test error response for missing pane_id
- [ ] Test error response for permission denied
- [ ] Test error response for iTerm2 not running
- [ ] Test error response includes fallback_path
- [ ] Test timeout handling (mock slow AppleScript)
- [ ] Test focus service unit tests with mocked subprocess
- [ ] Test AppleScript generation is correct

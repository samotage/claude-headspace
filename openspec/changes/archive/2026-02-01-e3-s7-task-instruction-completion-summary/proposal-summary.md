# Proposal Summary: e3-s7-command-instruction-completion-summary

## Architecture Decisions
- Command model enriched with `instruction` + `instruction_generated_at` fields; existing `summary` renamed to `completion_summary` (not a new field — rename preserves data)
- Instruction generation uses the same async thread pool pattern as existing turn/command summarisation
...
- Command lifecycle triggers instruction summarisation in `process_turn()` when creating a task from USER COMMAND
- Dashboard route adds `command_instruction` to the agent context dict; template displays it as primary line
- SSE introduces `instruction_summary` event type; existing `command_summary`/`turn_summary` events continue unchanged
- All `command.summary` → `command.completion_summary` renames done as a bulk find-and-replace pass across the codebase

## Files to Modify

### Models
- `src/claude_headspace/models/command.py` — rename `summary`→`completion_summary`, `summary_generated_at`→`completion_summary_generated_at`, add `instruction`, `instruction_generated_at`
...
- `src/claude_headspace/services/command_lifecycle.py` — trigger instruction summarisation on task creation from USER COMMAND
- `src/claude_headspace/services/priority_scoring.py` — update `command_summary` variable that reads from command model

### Routes
- `src/claude_headspace/routes/summarisation.py` — update field references from `summary` to `completion_summary`
- `src/claude_headspace/routes/dashboard.py` — update `get_command_summary()`, `_get_completed_command_summary()` field refs; add `command_instruction` to agent context
...
- `tests/services/test_command_lifecycle.py` — instruction trigger test
- `tests/services/test_priority_scoring.py` — updated field references
- All other test files referencing `command.summary` or `command.summary_generated_at`
...
- Turn summaries are intent-aware and include command instruction context
- Empty-text turns produce no summary (None returned)
- Agent card shows instruction (line 1) + latest turn summary (line 2)
- SSE pushes instruction updates independently from turn/command summaries
...
- **Priority scoring**: `priority_scoring.py` reads `recent_turn.summary` (turn summary, not command summary) — this is turn-level and unchanged, but the `command_summary` variable in the scoring prompt needs updating
- **Template caching**: Flask may cache templates; server restart required after template changes
- **SSE payload backward compat**: The `command_summary` SSE event payload currently sends `summary` key — decide whether to rename to `completion_summary` in payload or keep `summary` for JS compat
...
- Services: `inference_service.py`, `inference_cache.py`, `inference_rate_limiter.py`, `summarisation_service.py`, `command_lifecycle.py`, `priority_scoring.py`
...
### OpenSpec History
- `e3-s2-turn-command-summarisation` — Original summarisation capability (turn + command summaries, async processing, SSE broadcast)
...
## OpenSpec References
- proposal.md: openspec/changes/e3-s7-command-instruction-completion-summary/proposal.md
- tasks.md: openspec/changes/e3-s7-command-instruction-completion-summary/tasks.md
- spec.md (domain-models): openspec/changes/e3-s7-command-instruction-completion-summary/specs/domain-models/spec.md
- spec.md (summarisation): openspec/changes/e3-s7-command-instruction-completion-summary/specs/summarisation/spec.md
- spec.md (dashboard): openspec/changes/e3-s7-command-instruction-completion-summary/specs/dashboard/spec.md

...
## Purpose
Extends both agent creation paths (programmatic `create_agent()` and CLI `claude-headspace start`) with optional persona slug and previous_agent_id parameters, carrying them through the session environment and hook pipeline for downstream consumption by the SessionCorrelator (S8).

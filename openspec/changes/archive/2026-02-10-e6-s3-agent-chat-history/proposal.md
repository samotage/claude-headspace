...
- Each turn includes task_id for client-side task boundary detection
...
- Task boundary separators showing task instruction
...
- Affected code:
  ...
  - `static/voice/voice-app.js` — pagination, smart grouping, timestamps, task separators, ended agent UI
  - `static/voice/voice-api.js` — pagination params, ended agent transcript fetch
  - `static/voice/voice.css` — task separator styles, loading indicator, ended agent banner

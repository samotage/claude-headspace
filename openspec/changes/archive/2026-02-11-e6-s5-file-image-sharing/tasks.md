...
- [x] 2.1 Add `file_upload` configuration section to config.yaml with defaults (upload_dir, max_file_size_mb, max_total_storage_mb, retention_days, allowed types)
- [x] 2.2 Add `file_upload` config defaults to `config.py` so config loads cleanly when section is missing
- [x] 2.3 Create Alembic migration adding `file_metadata` JSONB column to `turns` table

### Turn Model

- [x] 2.4 Add `file_metadata` (JSONB, nullable) mapped column to `Turn` model in `turn.py`

### File Upload Service

- [x] 2.5 Create `src/claude_headspace/services/file_upload.py` with `FileUploadService` class:
...
- [x] 2.6 Register `FileUploadService` in `app.py` as `app.extensions["file_upload"]`
- [x] 2.7 Schedule cleanup: call `startup_sweep()` during app init, and register a background timer or hook for periodic cleanup
- [x] 2.8 Ensure upload directory is created on startup if it doesn't exist

### Voice Bridge Routes — Upload Endpoint

- [x] 2.9 Add `POST /api/voice/agents/<agent_id>/upload` route to `voice_bridge.py`:
...
- [x] 2.10 Add `GET /api/voice/uploads/<filename>` route to `voice_bridge.py`:
...
- [x] 2.11 Modify `POST /api/voice/command` to accept optional `file_path` field in JSON body
...
- [x] 2.12 Include `file_metadata` field in turn dicts returned by `GET /api/voice/agents/<id>/transcript`

### Voice API Client (JavaScript)

- [x] 2.13 Add `uploadFile(agentId, file, text)` method to `VoiceAPI` in `voice-api.js`:
...
- [x] 2.14 Add drop zone overlay HTML to `voice.html` inside `#screen-chat`
- [x] 2.15 Add dragenter/dragover/dragleave/drop event handlers in `voice-app.js`:
  - Show overlay on dragenter, hide on dragleave/drop
  - On drop: extract file, validate client-side (type + size), show pending preview
- [x] 2.16 Add drop zone CSS styles in `voice.css` (overlay, border highlight, icon)

### Chat UI — Clipboard Paste

- [x] 2.17 Add paste event handler on chat text input in `voice-app.js`:
...
- [x] 2.18 Add pending attachment preview area in `voice.html` (inside chat-input-bar, above textarea)
- [x] 2.19 Implement `_showPendingAttachment(file)` in `voice-app.js`:
...
- [x] 2.20 Add pending attachment CSS styles in `voice.css`

### Chat UI — Send with Attachment

- [x] 2.21 Modify chat form submit handler in `voice-app.js`:
...
- [x] 2.22 Modify `_createBubbleEl` in `voice-app.js` to check for `file_metadata` on turn:
...
- [x] 2.23 Add image thumbnail CSS styles in `voice.css`

### Chat UI — Non-Image File Cards

- [x] 2.24 Modify `_createBubbleEl` to render file card for non-image attachments:
  - Show file type icon (emoji or SVG), filename, file size (human-readable)
  - Clicking card opens/downloads the file
- [x] 2.25 Add file card CSS styles in `voice.css`

### Chat UI — Upload Progress & Error Feedback

- [x] 2.26 Add upload progress indicator in chat input area (progress bar or spinner)
- [x] 2.27 Add error toast/inline message rendering for upload errors:
  - Invalid file type: list accepted formats
  - File too large: show size limit
  - Upload failed: show retry option
- [x] 2.28 Add progress and error CSS styles in `voice.css`

## 3. Testing (Phase 3)

### Service Tests

- [x] 3.1 Create `tests/services/test_file_upload.py`:
...
- [x] 3.2 Create `tests/routes/test_voice_bridge_upload.py`:

...
- [x] 2.1.1 Create `src/claude_headspace/services/tmux_bridge.py` with:
...
- [x] 2.2.1 Create Alembic migration to add `tmux_pane_id` nullable string column to `agents` table
- [x] 2.2.2 Update Agent model (`models/agent.py`) to add `tmux_pane_id` mapped column

### 2.3 Hook Script

- [x] 2.3.1 Update `bin/notify-headspace.sh` to extract `$TMUX_PANE` env var and include `tmux_pane` field in every hook payload via jq

### 2.4 Hook Routes & Receiver

- [x] 2.4.1 Update `routes/hooks.py` — all 8 hook route handlers extract `tmux_pane = data.get("tmux_pane")`:
...
- [x] 2.4.2 Update `hook_receiver.py` — `process_session_start()` gains `tmux_pane_id: str | None = None` parameter; stores on agent and registers with availability tracker

### 2.5 Availability Tracker

- [x] 2.5.1 Update `commander_availability.py`:
...
- [x] 2.6.1 Update `routes/respond.py`:
...
- [x] 2.7.1 Update `app.py`: register `tmux_bridge` in `app.extensions["tmux_bridge"]`; update commander_availability init to not depend on commander_service
- [x] 2.7.2 Update `config.yaml`: replace `commander:` with `tmux_bridge:` section (health_check_interval: 30, subprocess_timeout: 5, text_enter_delay_ms: 100, sequential_send_delay_ms: 150)

### 2.8 Cleanup

- [x] 2.8.1 Delete `src/claude_headspace/services/commander_service.py`

## 3. Testing (Phase 3)

- [x] 3.1 Create `tests/services/test_tmux_bridge.py` — unit tests for send_text, send_keys, check_health, capture_pane, list_panes, error handling (mock subprocess)
- [x] 3.2 Update `tests/services/test_commander_availability.py` — update mocks from commander_service to tmux_bridge; verify tmux pane checks
- [x] 3.3 Update `tests/routes/test_respond.py` — update mocks for tmux_bridge; test tmux_pane_id validation; test TmuxBridgeErrorType mapping
- [x] 3.4 Update `tests/routes/test_hooks.py` — verify tmux_pane extraction and passthrough for all hook routes
- [x] 3.5 Delete `tests/services/test_commander_service.py`

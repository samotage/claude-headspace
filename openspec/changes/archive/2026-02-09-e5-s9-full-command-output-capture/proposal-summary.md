...
- Store full text on the Command model (not Turn) — the PRD treats this as command-level data
...
- Project view uses expandable accordion pattern consistent with existing command card rendering

## Implementation Approach
- Minimal backend changes: two new fields on Command model, capture at two existing lifecycle methods
- The full command text is already available in `process_turn()` as the `text` parameter (line 324/370 of command_lifecycle.py)
- The full agent output is already available in `complete_task()` as the `agent_text` parameter (line 253/288 of command_lifecycle.py)
- No changes needed to card_state.py — full text is intentionally excluded from `build_card_state()`
- New endpoint `/api/commands/<id>/full-text` returns both fields on demand
- Frontend fetches on click/expand, not on initial page load

## Files to Modify

### Models
- `src/claude_headspace/models/command.py` — add `full_command` and `full_output` Text fields

### Migrations
- `migrations/versions/` — new Alembic migration adding two nullable Text columns to `commands` table

### Services
- `src/claude_headspace/services/command_lifecycle.py` — persist full text in `process_turn()` and `complete_task()`
...
### Routes
- `src/claude_headspace/routes/projects.py` — add `GET /api/commands/<id>/full-text` endpoint
...
- Full text NOT in `/api/agents/<id>/commands` response
...
### Related Files
- Models: `src/claude_headspace/models/command.py` (Command model with instruction, completion_summary fields)
- Services: `src/claude_headspace/services/command_lifecycle.py` (CommandLifecycleManager)
...
- Routes: `src/claude_headspace/routes/projects.py` (agent tasks API, command turns API)
...
- JS: `static/js/project_show.js` (project view with agent/command/turn drill-down)
...
## Testing Strategy
- Unit tests for Command model field persistence
- Unit tests for `process_turn()` full_command capture
- Unit tests for `complete_task()` full_output capture
- Route tests for the new `/api/commands/<id>/full-text` endpoint
...
- spec.md: openspec/changes/e5-s9-full-command-output-capture/specs/command-model/spec.md

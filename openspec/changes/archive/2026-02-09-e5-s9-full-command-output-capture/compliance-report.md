...
|-----------|--------|-------|
| Every task with a user command has `full_command` persisted | ✓ | `process_turn()` sets `new_task.full_command = text` when text is truthy |
| Every completed task has `full_output` persisted | ✓ | `complete_task()` sets `task.full_output = agent_text` when agent_text is truthy |
...
| Full text NOT in `/api/agents/<id>/tasks` response | ✓ | Explicit serialization on lines 619-629 of projects.py; test verifies |
...
- **Tasks Completed:** 21/21 complete (Phase 1: 3, Phase 2: 12, Phase 3: 6)

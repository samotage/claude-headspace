...
The dashboard currently shows AI-generated summaries of task instructions and completion results, but discards the full user command text and agent final message. Users cannot review complete agent responses without returning to the terminal, limiting the dashboard's utility on mobile devices and remote access scenarios.

## What Changes

- Add two new `Text` fields to the Task model: `full_command` and `full_output`
- Persist the full user command text when a task is created (during `process_turn` for USER COMMAND)
- Persist the full agent final message when a task completes (during `complete_task`)
- Add an on-demand API endpoint to fetch full text for a task (not included in SSE card_refresh payloads)
...
- Affected specs: task-model, task-lifecycle, hook-receiver, dashboard-ui, project-show
- Affected code:
  - `src/claude_headspace/models/task.py` — add `full_command` and `full_output` fields
  - `migrations/versions/` — new Alembic migration for the two columns
  - `src/claude_headspace/services/task_lifecycle.py` — persist full text during create_task/complete_task
  - `src/claude_headspace/services/hook_receiver.py` — pass full command text through to task lifecycle

...
### NFR: Hook Processing Latency

- **WHEN** any hook event is processed
- **THEN** processing adds less than 100ms latency to the existing pipeline

### NFR: Incremental Transcript Parsing

- **WHEN** transcript files are read
- **THEN** the system uses incremental parsing (track file position)
- **AND** does not re-read the entire file on each event

### NFR: Rate Limiting Compliance

- **WHEN** inference calls are made for question classification
- **THEN** they respect existing rate limits (30 calls/min, 50k tokens/min)

### NFR: Database Safety

- **WHEN** new database operations are performed
- **THEN** they use existing session/transaction patterns
- **AND** avoid FK race conditions (as fixed in EventWriter)

### NFR: Test Coverage

- **WHEN** new endpoints and services are created
- **THEN** they have unit tests, route tests, and integration tests
- **AND** follow the existing three-tier test architecture

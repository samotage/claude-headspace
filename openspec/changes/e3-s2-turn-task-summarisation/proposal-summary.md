# Proposal Summary: e3-s2-turn-task-summarisation
...
- Summary fields added directly to Turn and Task models (not a separate SummaryResult table) for simplicity and query efficiency

## Implementation Approach
- Add `summary` (Text, nullable) and `summary_generated_at` (DateTime, nullable) columns to both Turn and Task models
- Create `SummarisationService` class that wraps `InferenceService.infer()` with prompt building and result persistence
- Integrate summarisation triggers in `TaskLifecycleManager.process_turn()` and `complete_task()` — fire-and-forget async calls
...
### New Files
- `src/claude_headspace/services/summarisation_service.py` — SummarisationService with turn and task summarisation
- `src/claude_headspace/routes/summarisation.py` — Blueprint with POST /api/summarise/turn/<id> and POST /api/summarise/task/<id>
...
- `src/claude_headspace/models/task.py` — Add summary and summary_generated_at columns
- `src/claude_headspace/services/task_lifecycle.py` — Add summarisation triggers after process_turn() and complete_task()
...
- Task summaries generated automatically on task completion
- Cached results returned for identical content without duplicate inference calls
- POST /api/summarise/turn/<id> and /api/summarise/task/<id> return correct summaries
...
- The TaskLifecycleManager currently doesn't have access to Flask `current_app` — we need to pass the app reference or use a service registry pattern
...
- InferenceService.infer() accepts turn_id, task_id, agent_id, project_id kwargs — use them for InferenceCall logging
- Migration must chain from current head `c5d6e7f8a9b0` (add_inference_calls_table)
- The dashboard route's `get_task_summary()` helper currently returns first 100 chars of most recent turn text — needs updating to prefer the Turn.summary field
- Agent card template uses `task_summary` variable — keep the same variable name but populate with summary when available
- Do NOT modify the Broadcaster class itself — just call `broadcast()` with new event types

## Git Change History

### Related Files
- Models: `turn.py`, `task.py`, `inference_call.py`, `agent.py`, `project.py`
- Services: `task_lifecycle.py`, `inference_service.py`, `broadcaster.py`
...
- Integration tests: summary field persistence on Turn and Task models
- Unit tests for TaskLifecycleManager integration: verify summarisation is triggered after turn processing and task completion
- Full suite regression: all existing tests must pass

## OpenSpec References
- proposal.md: openspec/changes/e3-s2-turn-task-summarisation/proposal.md
- tasks.md: openspec/changes/e3-s2-turn-task-summarisation/tasks.md
- spec.md: openspec/changes/e3-s2-turn-task-summarisation/specs/summarisation/spec.md

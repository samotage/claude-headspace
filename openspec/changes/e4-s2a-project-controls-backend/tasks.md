...
- [ ] 2.1.1 Add `description` (Text, nullable), `inference_paused` (Boolean, not null, default false), `inference_paused_at` (DateTime(timezone=True), nullable), `inference_paused_reason` (Text, nullable) columns to `projects` table
- [ ] 2.1.2 Update Project model in `models/project.py` with new fields
- [ ] 2.1.3 Run migration and verify schema

### 2.2 Project CRUD Routes

- [ ] 2.2.1 Create `src/claude_headspace/routes/projects.py` blueprint with:
...
- [ ] 2.2.2 Register `projects_bp` in `app.py` `register_blueprints()`

### 2.3 Project Settings Routes

- [ ] 2.3.1 Add to `routes/projects.py`:
...
- [ ] 2.4.1 Broadcast `project_changed` SSE event (action: created/updated/deleted, project_id) on CRUD operations
- [ ] 2.4.2 Broadcast `project_settings_changed` SSE event (project_id, inference_paused, inference_paused_at) on settings changes

### 2.5 Disable Auto-Discovery

- [ ] 2.5.1 In `session_correlator.py` `_create_agent_for_session()`: remove Project auto-creation; raise `ValueError` with message directing user to register project when working directory doesn't match registered project
- [ ] 2.5.2 In `routes/sessions.py` `create_session()`: remove Project auto-creation; return 404 with message directing user to register project when project_path doesn't match registered project
- [ ] 2.5.3 Verify error messages include rejected path and reference to `/projects` management page

### 2.6 Inference Gating

- [ ] 2.6.1 In `summarisation_service.py`: add inference pause check before `summarise_turn()`, `summarise_task()`, `summarise_instruction()` — return None if project.inference_paused, with debug log
- [ ] 2.6.2 In `priority_scoring.py` `score_all_agents()`: filter out agents from paused projects in query; skip scoring entirely if all agents belong to paused projects

## 3. Testing (Phase 3)

### 3.1 Project CRUD Route Tests

- [ ] 3.1.1 Create `tests/routes/test_projects.py`
...
- [ ] 3.2.1 Add to `tests/routes/test_projects.py`
...
- [ ] 3.3.1 Update `tests/services/test_session_correlator.py` — test ValueError raised for unregistered project
- [ ] 3.3.2 Update `tests/routes/test_sessions.py` — test 404 returned for unregistered project

### 3.4 Inference Gating Tests

- [ ] 3.4.1 Create `tests/services/test_inference_gating.py`
...
- [ ] 3.5.1 Test project CRUD operations broadcast project_changed events
- [ ] 3.5.2 Test settings changes broadcast project_settings_changed events
...
- [ ] 4.4 Auto-discovery fully disabled (no Project auto-creation in codebase)
- [ ] 4.5 Inference gating verified for all call paths

...
_PROMPT_TEMPLATES: dict[str, str] = {
    ...
    #
    # STYLE RULE: Summaries are command board entries. Use imperative/direct form.
    # NEVER describe the user ("The user wants...", "The user is...").
    # Good: "Fix login validation bug"  Bad: "The user wants to fix a bug"
    "turn_command": (
        "{instruction_context}"
        "Command: {text}\n\n"
        "Write a command board entry (~18 tokens) stating the goal of this command. "
    ...
    "turn_question": (
        "{instruction_context}"
        "Agent asked: {text}\n\n"
        "Write a command board entry (~18 tokens) stating what the agent needs to know. "
    ...
    "turn_completion": (
        "{instruction_context}"
        "Agent output: {text}\n\n"
        "Write a command board entry (~18 tokens) stating what was accomplished. "
    ...
    "turn_progress": (
        "{instruction_context}"
        "Agent output: {text}\n\n"
        "Write a command board entry (~18 tokens) stating current progress. "
    ...
    "turn_answer": (
        "{instruction_context}"
        "Response: {text}\n\n"
        "Write a command board entry (~18 tokens) stating what was confirmed or provided. "
    ...
    "turn_end_of_command": (
        "{instruction_context}"
        "Final output: {text}\n\n"
        "Write a command board entry (~18 tokens) stating the final outcome. "
    ...
    "turn_default": (
        "{instruction_context}"
        "{actor}: {text}\n\n"
        "Write a command board entry (~18 tokens) stating the action taken or requested. "
    ...
    "command_completion_from_instruction": (
        "Command: {instruction}\n\n"
        "The command was completed but no agent output was captured. "
        "Write a command board entry (~18 tokens) stating what was likely accomplished. "
        "Use past tense. NEVER start with 'The user' or 'The agent'. "
        "Output ONLY the entry — no preamble, labels, or commentary."
    ),
    ...
    "instruction": (
        ...
        "Write a command board entry (~18 tokens) stating the goal. "
    ...
    "priority_scoring": (
        ...
        "3. Command duration (15%) - Longer running commands may need attention\n"
    ...
    "turn_frustration": (
        "{instruction_context}"
        "Message: {text}\n\n"
        "1. Write a command board entry (~18 tokens) stating the goal or action. "
...
def build_prompt(prompt_type: str, **context) -> str:
    ...
    Args:
        prompt_type: Key into _PROMPT_TEMPLATES (e.g. "turn_command",
            "command_completion", "priority_scoring").

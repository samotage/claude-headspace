...
class StateMachine:
    """
    Stateless state machine for task state transitions.

    This class provides the transition logic without storing any state.
    All state is passed in as arguments and returned as results.
    """
    ...
    def transition(
        ...
        """
        result = validate_transition(from_state, actor, intent)

        if not result.valid:
            logger.warning(
                f"Invalid state transition rejected: {result.reason} "
                f"(from={from_state.value}, actor={actor.value}, intent={intent.value})"
            )
        else:
            logger.debug(
                f"Valid state transition: {from_state.value} -> {result.to_state.value} "
    ...
    def can_transition(
        ...
        """
        Check if a transition would be valid without logging.
